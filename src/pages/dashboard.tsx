...
import { getAccountBalances } from 'apiSdk/account-balances';
import { AccountBalanceInterface } from 'interfaces/account-balance';
import { getUserAuthentications } from 'apiSdk/user-authentications';
...
import { AccountBalanceListPage as ShowCaseList } from 'pages/account-balances';

function DashboardPage() {
  const { session } = useSession();

  const accountBalanceStat = useSWR<PaginatedInterface<AccountBalanceInterface>>(
    () => `/account-balances`,
    () => getAccountBalances({ limit: 1, offset: 0 }),
  );

  const userAuthenticationStat = useSWR<PaginatedInterface<UserAuthenticationInterface>>(
    () => `/user-authentications`,
    () => getUserAuthentications({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: accountBalanceStat?.data?.totalCount,
      isLoading: accountBalanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Authentications',
      result: userAuthenticationStat?.data?.totalCount,
      isLoading: userAuthenticationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`account-balances`} passHref legacyBehavior>
                  <Button
